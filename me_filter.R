...
# Conto il numeri eventi a cui ogni utente ha partecipato
eventi_per_membro <- sqldf("SELECT member_id, COUNT(*) AS 'Eventi'
                           FROM data GROUP BY member_id")
hist(eventi_per_membro$Eventi, nclass=250, xlim=c(0,20),
     main=paste("Istogramma di",NROW(eventi_per_membro),"eventi"))
table(eventi_per_membro$Eventi)[1:30]
prop.table(table(eventi_per_membro$Eventi)[1:30])

me_data <- filtro_me(data, 60)$data
me_sociomatrice <- sociomatrice_membri(me_data)

me.net <- graph_from_adjacency_matrix(as.matrix(me_sociomatrice),
                                      mode="undirected", weighted=T, diag=F)
plot(me.net)
hist(degree(me.net), nclass=30)
hist(betweenness(me.net), nclass=100)

# ---------------------------------------------------------------------------- #

Y <- as.matrix(me_sociomatrice)
...
anova(mod.anova1)
anova(mod.anova2) 
summary(mod.anova1) # R2 = 0.03223
summary(mod.anova2) # R2 = 0.1112
...
mu.hat             # ANOVA - media generale  0.7225054
mean(fit_SRM$BETA) #   SRM - media generale  0.7189413
...
# 0.8080730  0.8080730  1.0000000  0.1265582
