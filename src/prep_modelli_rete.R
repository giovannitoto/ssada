...
# Funzioni utili per modelli applicati a una sociomatrice, vettorizzata e non

variabili_ame <- function(sociomatrice, events_groups) {
...
variabili_modelli <- function(sociomatrice, same_group, same_cat, Xn) {
  ...
  dati_vett[,1] <- as.numeric(as.character(dati_vett[,1]))
  dati_vett[,2] <- as.numeric(as.character(dati_vett[,2]))
  dati_vett[,7] <- as.numeric(as.character(dati_vett[,7]))
...
{ # Ritorna una matrice con i valori predetti per ogni cella della sociomatrice.
  # per ottenere una pred.v adatta (puo' cambiare a seconda del modello) bisogna
  # prima scrivere:
  #     pred.v <- c(predict(mod))  oppure fitted(mod)
  
  names(pred.v) <- NULL  # passaggio necessario
