...
data2 <- data[data$category_name=="Tech",]

dd <- sqldf("SELECT event_id, COUNT(*) AS 'partecipanti'
             FROM data2 GROUP BY event_id")
dd <- merge(dd, events_groups[,c("event_id","group_name")], by="event_id")
dd$group_name <- as.factor(dd$group_name)
colnames(dd)[3] <- "group"

class.c <- c("Agile Nashville User Group",
             "Code for Nashville",
...
new_levels <- levels(dd$group)
for (c in 1:length(levels(dd$group))) {
  if (!(levels(dd$group)[c] %in% class.c)) {
    new_levels[c] <- "Other"
  }
}
levels(dd$group) <- new_levels
...
p2 <- ggplot(dd[dd$partecipanti>=20,], aes(x=partecipanti)) +
  geom_histogram( binwidth=2, fill="#7bf87b", color="#e9ecef", alpha=1) +
  labs(x="partecipanti", y = "conteggio") +
  coord_cartesian(xlim = c(30, 141)) +
  scale_x_continuous(breaks=c(2030,40,50,60,70,100,146)) +
  scale_y_continuous(breaks=c(0,10,20,30,40,50,60,70))

p3 <- ggplot(dd, aes(partecipanti, fill=group)) +
  geom_histogram( binwidth=3, color="#e9ecef", alpha=1) +
  labs(x="partecipanti", y = "conteggio") +
  coord_cartesian(xlim = c(0, 141)) +
  scale_x_continuous(breaks=seq(0,150,20)) +
...
p4 <- ggplot(dd[dd$partecipanti>=20,], aes(x=partecipanti, fill=group)) +
  geom_histogram( binwidth=2, color="#e9ecef", alpha=1) +
  labs(x="partecipanti", y = "conteggio") +
  coord_cartesian(xlim = c(20, 141)) +
  scale_x_continuous(breaks=c(20,30,40,50,60,70,100,141)) +
...
data <- data[data$category_name %in% cat_list,]
...
class.c <- c("20's & 30's Women looking for girlfriends",
             "20s in Nashville",
...
p2 <- ggplot(dd[dd$partecipanti>=20,], aes(x=partecipanti)) +
  geom_histogram( binwidth=2, fill="#7bf87b", color="#e9ecef", alpha=1) +
  labs(x="partecipanti", y = "conteggio") +
  coord_cartesian(xlim = c(30, 92)) +
  scale_x_continuous(breaks=c(20,30,40,50,60,70,100,146)) +
  scale_y_continuous(breaks=c(0,10,20,30,40,50,60,70))
...
p4 <- ggplot(dd[dd$partecipanti>=20,], aes(x=partecipanti, fill=group)) +
  ...
  labs(x="partecipanti", y = "conteggio") +
  coord_cartesian(xlim = c(20, 141)) +
  scale_x_continuous(breaks=c(20,30,40,50,60,70,92)) +
  scale_y_continuous(breaks=c(0,10,20,30,40,50,60,70)) +
  scale_fill_manual(breaks=class.c,values=COL) +
